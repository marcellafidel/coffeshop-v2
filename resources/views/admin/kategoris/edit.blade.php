...
<style>
    ...
    .form-group input:focus { outline:none; border-color:#A8C8E8; background:#fff; }
...
<div class="form-card">
    <form action="{{ route('admin.kategoris.update', $kategori) }}" method="POST">
        @csrf @method('PUT')
        <div class="icon-preview" id="iconPreview">{{ $kategori->icon }}</div>
        <div class="form-group">
            <label>Icon (Emoji)</label>
            <input type="text" name="icon" value="{{ $kategori->icon }}" maxlength="4" oninput="document.getElementById('iconPreview').textContent = this.value || '☕'">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea name="deskripsi" rows="2">{{ $kategori->deskripsi }}</textarea>
        </div>
